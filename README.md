...
**목적**: 매일 시장 종료 시간(18:15)에 키움 조건 검색 API를 통해 "대왕개미단타" 조건을 만족하는 종목들을 자동으로 검색하여 rec_stocks에 저장

**구현 내용**:

1. **스케줄러 함수 추가** (`back/app/scheduler.py`):
   - `update_dae_wangkaemi_stocks_job()`: 대왕개미 단타론 조건으로 종목 검색 및 업데이트
   - 신고가 따라잡기와 동일한 로직 사용

2. **스케줄 설정**:
   - **시간**: 매일 18:15 (신고가 따라잡기는 18:10)
   - **제외 요일**: 토요일, 일요일
   - **실행 요일**: 월요일 ~ 금요일
   - **조건식**: '대왕개미단타' (키움 조건 검색 이름)
   - **알고리즘 ID**: 2 (algorithm 테이블의 '대왕개미 단타론')
...
3. **동작 흐름**:
   ...
   RecommendationService 생성
      ↓
   조건명 '대왕개미단타'로 조건 ID 조회
      ↓
   키움 API에서 종목 검색
      ↓
   검색된 종목들을 rec_stocks에 저장 (누적, algorithm_id=2)
      ↓
   로그에 결과 기록
   ```
...
4. **데이터 관리**:
   ...
   - `algorithm_id=2`로 신고가 따라잡기(algorithm_id=1)와 별도 관리
...
**주의사항**:
- 두 조건 모두 토일요일 제외하고 평일(월-금)에만 실행
- 18:15 스케줄이 18:10의 신고가 따라잡기 작업 완료 후에 실행되도록 5분 차이 설정
